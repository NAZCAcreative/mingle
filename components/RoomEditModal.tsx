    ...
    <div className="fixed inset-0 z-[1200] bg-ink/35 px-4 py-6" role="dialog" aria-modal="true" aria-label="방 정보 수정">
      <div className="mx-auto flex max-h-[calc(100vh-48px)] w-full max-w-[430px] flex-col overflow-hidden rounded-card bg-white shadow-card">
        <div className="flex items-center justify-between border-b border-blush px-5 py-4">
          <h2 className="text-xl font-semibold text-ink">방 정보 수정</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4">
          <label className="block">
            <span className="text-sm font-semibold text-ink">방 제목</span>
            <input
              value={title}
              onChange={(event) => setTitle(event.target.value)}
              maxLength={60}
              className="mt-1.5 h-[50px] w-full rounded-button border border-blush bg-cream px-4 font-medium text-ink outline-none focus:border-mingle"
            />
          </label>

          <label className="block">
            <span className="text-sm font-semibold text-ink">요약</span>
            <textarea
              value={summary}
              onChange={(event) => setSummary(event.target.value)}
              maxLength={200}
              rows={3}
              className="mt-1.5 w-full resize-none rounded-button border border-blush bg-cream px-4 py-3 font-medium text-ink outline-none focus:border-mingle"
            />
          </label>

          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-sm font-semibold text-ink">시간</span>
              <input
                value={meetingTime}
            ...
            <label className="block">
              <span className="text-sm font-semibold text-ink">장소</span>
              <input
                value={destination}
          ...
          <label className="block">
            <span className="text-sm font-semibold text-ink">최대 인원</span>
            <input
              value={maxPeople}
          ...
          {error ? <p className="text-sm font-medium text-mingle">{error}</p> : null}
        </div>
        ...
        <div className="border-t border-blush px-5 py-4">
          <button
            ...
            className="h-[54px] w-full rounded-button bg-mingle text-[17px] font-semibold text-white disabled:bg-neutral-300"
          >
            {saving ? "저장 중..." : "저장하기"}
          </button>
        </div>
