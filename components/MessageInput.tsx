...
export function MessageInput({ disabled, onSend }: { disabled: boolean; onSend: (content: string) => Promise<void> }) {
  ...
  return (
    ...
    >
      <input disabled={disabled} value={value} onChange={(event) => setValue(event.target.value)} placeholder={disabled ? "닉네임 입력 후 대화할 수 있어요" : "메시지를 입력하세요"} className="h-[52px] min-w-0 flex-1 rounded-button border border-blush bg-white px-4 text-[16px] font-bold outline-none focus:border-mingle disabled:bg-neutral-100" />
